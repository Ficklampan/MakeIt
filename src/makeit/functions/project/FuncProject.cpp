...
makeit::Function* makeit::function::make_project()
{
  return new Function({
	  ...
	  StructArg::Arg("outname", false, new Argument(Variable::STRING)),
	  ...
	  StructArg::Arg("type", false, new Argument(Variable::STRING))
	  })
      }, exec_project);
}
int makeit::function::exec_project(void* ptr, std::vector<Variable*> &args)
{
  ...
  for (auto &[key, value] : *st)
  {
    ...
    else if (key.compare("outname") == 0)
      project->config.out_name = *value->as_string();
	...
	project->config.out_type = BuildConfig::EXECUTABLE;
      else if (type.compare("static_library") == 0)
	project->config.out_type = BuildConfig::STATIC_LIBRARY;
      else if (type.compare("shared_library") == 0)
	project->config.out_type = BuildConfig::SHARED_LIBRARY;
      else
