...
int main(int argc, char** argv)
{
  ...
  const char* directory = strpathfix(argv[1]);
  const char* filepath = strjoin(directory, "MakeIt.txt");
  printf("==> Parsing `%s`\n", filepath);
...
int makeit_parse_data(makeit_project* project, const char* data)
{
  ...
  for (uint32_t i = 0; i < data_length; i++)
  {
    ...
    {
      data_type = 0;
      if (makeit_process_functions(project, func->str, elements) != 1)
        return 0;
    ...
    else if (data_type == 2 && c == '\n')
    {
      if (element->length > 0)
      {
        ...
        if (element->length > 0)
        {
          if (makeit_init_value(project, &element->str) != 1)
            return 0;
